...
HuskyHighlevelController::HuskyHighlevelController(ros::NodeHandle& nodeHandle) : nodeHandle_(nodeHandle){
  ...
  nodeHandle.getParam("/husky_highlevel_controller/scan_subscriber_topic", scan_subscriber_topic);
  nodeHandle.getParam("/husky_highlevel_controller/scan_subscriber_queue_size", scan_subscriber_queue_size);
  ...
  scan_subscriber_ = nodeHandle_.subscribe(scan_subscriber_topic, scan_subscriber_queue_size, &HuskyHighlevelController::scanCallback, this);
...
void HuskyHighlevelController::scanCallback(const sensor_msgs::LaserScan &scan_msg) {
  ...
  for (float vec:ranges) {//interate vec (index for element of the vector) from vector called ranges
